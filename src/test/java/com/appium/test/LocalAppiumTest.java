...
public class LocalAppiumTest {
	...
	public static void main(String[] args) {
		...
		try {
			...
			new WebDriverWait(driver, 90L)
			.until(ExpectedConditions.presenceOfElementLocated(MobileBy.id("eu.uvita:id/activity_import_encrypted_private_key_welcome_no_key"))).click();
			
			MobileElement deleteData = driver.findElement(MobileBy.id("eu.uvita:id/activity_no_key_available_button"));
			deleteData.click();
			
			MobileElement yesDeleteMyData = driver.findElement(MobileBy.id("eu.uvita:id/fragment_import_encrypted_private_key_delete_data_delete_button"));
			yesDeleteMyData.click();
			
			//wait for skip and click
			new WebDriverWait(driver, 90L)
			.until(ExpectedConditions.presenceOfElementLocated(MobileBy.id("eu.uvita:id/activity_export_private_key_skip"))).click();
			...
			new WebDriverWait(driver, 90L)
					.until(ExpectedConditions.presenceOfElementLocated(MobileBy.xpath("//android.widget.ImageButton[@content-desc='Navigate up']"))).click();
